...
for m = [0,n]
kernel_matrix(:,1+m) = interpolate_matrix(1+2*m,1+m)*function_matrix(:,1+2*m)+...
    + interpolate_matrix(n+1,1)*function_matrix(:,n+1+2*m);
for k = 2:n-2
    kernel_matrix(:,k+m) = interpolate_matrix(k+2*m,k+m)*function_matrix(:,k+2*m)+...
...
kernel_matrix(:,n-1+m) = interpolate_matrix(n-2+2*m,n-1+m)*function_matrix(:,n-2+2*m)+...
...
kernel_matrix(:,n+m) = interpolate_matrix(n-1+2*m,n+m)*function_matrix(:,n-1+2*m)+...

function [pressurerow, shearrow] = pressure_cubic_gunnar(z,x)
...
%uses (cx+d+e/x+f/x^2) for g at infinity
%uses (bx^2+cx+d+e/x+f/x^2) for h at infinity

%takes as input two rows of interpolate_cubic
%first 4*(n-1) data for g
%second 4*(n-1) data for h
%last 9 data for infinity
%has data in batches of 4, for a,b,c,d respectively
...
pressurerow = zeros(1,8*(n-1)+8);
shearrow = zeros(1,8*(n-1)+8);
...
for i = t+1:n-1
    ...
    [lK11x2, lK11x1, lK11x0, lK11xmin1, lK11xmin2, ...
    lK12x2, lK12x1, lK12x0, lK12xmin1, lK12xmin2, ...
    lK21x2, lK21x1, lK21x0, lK21xmin1, lK21xmin2, ...
    lK22x2, lK22x1, lK22x0, lK22xmin1, lK22xmin2] = x2_to_xminus2_kernel(lx,z);

    [uK11x2, uK11x1, uK11x0, uK11xmin1, uK11xmin2, ...
    uK12x2, uK12x1, uK12x0, uK12xmin1, uK12xmin2, ...
    uK21x2, uK21x1, uK21x0, uK21xmin1, uK21xmin2, ...
    ...
    pressurerow(4*(n-1)+4*(i-1)+1) = uK12x2 - lK12x2;
    pressurerow(4*(n-1)+4*(i-1)+2) = uK12x1 - lK12x1;
    pressurerow(4*(n-1)+4*(i-1)+3) = uK12x0 - lK12x0;
    pressurerow(4*(n-1)+4*(i-1)+4) = uK12xmin1 - lK12xmin1;
    ...
    shearrow(4*(n-1)+4*(i-1)+1) = uK22x2 - lK22x2;
    shearrow(4*(n-1)+4*(i-1)+2) = uK22x1 - lK22x1;
    shearrow(4*(n-1)+4*(i-1)+3) = uK22x0 - lK22x0;
    shearrow(4*(n-1)+4*(i-1)+4) = uK22xmin1 - lK22xmin1;
end

%does the panel at infinity: fiddly!!!!
lx = x(n);
ux = infinity;

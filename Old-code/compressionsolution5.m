...
inpoint = (0.5:1:n-0.5)/(n+1)./(1-(0.5:1:n-0.5)/(n+1));
%outpoint = (1:n)/(n+1)./(1-(1:n)/(n+1));
%vector of h points
...
%soln
partsoln = eqnmatrix\inhomog;

nullsoln = null(eqnmatrix);

lambda = -10^3:10:10^3;

A = partsoln*ones(1,length(lambda)) + nullsoln*lambda;

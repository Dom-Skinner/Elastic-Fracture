...
%at infinity uses an ax^2 + bx + c + d/x^2 expansion for h'
%at infinity uses an ax + b + c/x^2 expansion for g'
...
Pload = 0; %(2*pi)^(3/2)/(8*pi);
Mload = 1; %(2*pi)^(3/2)/(8*pi);
Nload = 0;

%vector of h' points
inpoint = tan((0.5:1:n-0.5)*(pi/(2*(n+1))));
%vector of h points
outpoint = tan((1:n)*(pi/(2*(n+1))));
...
%use ax + b + c/x^2 for g'
gcoeffmatrix = powerseriescoeffs(inpoint(n-2:n));
interpol(4*(n-1)+2:4*(n-1)+4,n-2:n) = gcoeffmatrix;

%use ax^2 + bx + c + d/x^2 for h'
hcoeffmatrix = powerseriescoeffs(inpoint(n-3:n));
interpol(4*(n-1)+5:4*(n-1)+8,2*n-3:2*n) = hcoeffmatrix;
    


%creates a matrix to find pressure
Kmatrix = zeros(2*(n-2), 4*(n-1)+8);
for i = 1:n-2
    x = outpoint(i);
    ...
    lK22cint, lK22dint, lK22fint, lK22hint] = quadinvsquarekernelmatrices(lx,x);

    [uK11aint, uK11bint, uK11eint, uK11gint, ...
    uK12cint, uK12dint, uK12fint, uK12hint, ...
    uK21aint, uK21bint, uK21eint, uK21gint, ...
    uK22cint, uK22dint, uK22fint, uK22hint] = quadinvsquarekernelmatrices(ux,x);

    %Kmatrix(i, 4*(n-1)+1) = uK11eint - lK11eint;
    Kmatrix(i, 4*(n-1)+2) = uK11aint - lK11aint;
    Kmatrix(i, 4*(n-1)+3) = uK11bint - lK11bint;
    Kmatrix(i, 4*(n-1)+4) = uK11gint - lK11gint;
    
    Kmatrix(i, 4*(n-1)+5) = uK12fint - lK12fint;
    Kmatrix(i, 4*(n-1)+6) = uK12cint - lK12cint;
    Kmatrix(i, 4*(n-1)+7) = uK12dint - lK12dint;
    Kmatrix(i, 4*(n-1)+8) = uK12hint - lK12hint;

    %Kmatrix(n-2+i, 4*(n-1)+1) = uK21eint - lK21eint;
    Kmatrix(n-2+i, 4*(n-1)+2) = uK21aint - lK21aint;
    Kmatrix(n-2+i, 4*(n-1)+3) = uK21bint - lK21bint;
    Kmatrix(n-2+i, 4*(n-1)+4) = uK21gint - lK21gint;
    
    Kmatrix(n-2+i, 4*(n-1)+5) = uK22fint - lK22fint;
    Kmatrix(n-2+i, 4*(n-1)+6) = uK22cint - lK22cint;
    Kmatrix(n-2+i, 4*(n-1)+7) = uK22dint - lK22dint;
    Kmatrix(n-2+i, 4*(n-1)+8) = uK22hint - lK22hint;
...
%
inhomog(2*n-3) = (-Pload+6*Mload-6*Nload); %g'
inhomog(2*n-2) = (-6*Nload); %g''
inhomog(2*n-1) = (12*Mload+12*Nload); %h''
inhomog(2*n) = (6*Nload); %h'''

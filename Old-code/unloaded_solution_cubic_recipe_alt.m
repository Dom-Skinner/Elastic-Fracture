...
g_infty_coeffs(2,1:n+2) = [2*x(n), 1, 0, -x(n)^(-2)]*interpol_matrix(4*(n-2)+1:4*(n-2)+4,:);
g_infty_coeffs(3,1:n+2) = [2, 0, 0, 2*x(n)^(-3)]*interpol_matrix(4*(n-2)+1:4*(n-2)+4,:);
...
h_infty_coeffs(1,n) = 1;
h_infty_coeffs(2,1:n+2) = [2, 0, 0, 2*x(n)^(-3)]*interpol_matrix(4*(n-2)+1:4*(n-2)+4,:);
h_infty_coeffs(3,n+2) = 1;
...
for j = 1:n
    [pressurerow, shearrow] = pressure_cubic_recipe_alt(z(j),x);

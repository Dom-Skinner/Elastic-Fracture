function [pressurerow, shearrow] = pressure_cubic_recipe(z,x)
...
for i = t:n-1
    ...
    [lK11x3, lK11x2, lK11x1, lK11x0, ...
    lK12x3, lK12x2, lK12x1, lK12x0, ...
    lK21x3, lK21x2, lK21x1, lK21x0, ...
    lK22x3, lK22x2, lK22x1, lK22x0] = x3_to_x0_kernel(lx,z);

    [uK11x3, uK11x2, uK11x1, uK11x0, ...
    uK12x3, uK12x2, uK12x1, uK12x0, ...
    uK21x3, uK21x2, uK21x1, uK21x0, ...
    uK22x3, uK22x2, uK22x1, uK22x0,] = x3_to_x0_kernel(ux,z);

    pressurerow(4*(i-1)+2) = uK11x2 - lK11x2;
    pressurerow(4*(i-1)+3) = uK11x1 - lK11x1;
    pressurerow(4*(i-1)+4) = uK11x0 - lK11x0;
    pressurerow(4*(i-1)+1) = uK11x3 - lK11x3;
    
    pressurerow(4*(n-1)+4*(i-1)+2) = uK12x2 - lK12x2;
    pressurerow(4*(n-1)+4*(i-1)+3) = uK12x1 - lK12x1;
    pressurerow(4*(n-1)+4*(i-1)+4) = uK12x0 - lK12x0;
    pressurerow(4*(n-1)+4*(i-1)+1) = uK12x3 - lK12x3;
    
    shearrow(4*(i-1)+2) = uK21x2 - lK21x2;
    shearrow(4*(i-1)+3) = uK21x1 - lK21x1;
    shearrow(4*(i-1)+4) = uK21x0 - lK21x0;
    shearrow(4*(i-1)+1) = uK21x3 - lK21x3;
    
    shearrow(4*(n-1)+4*(i-1)+2) = uK22x2 - lK22x2;
    shearrow(4*(n-1)+4*(i-1)+3) = uK22x1 - lK22x1;
    shearrow(4*(n-1)+4*(i-1)+4) = uK22x0 - lK22x0;
    shearrow(4*(n-1)+4*(i-1)+1) = uK22x3 - lK22x3;
end

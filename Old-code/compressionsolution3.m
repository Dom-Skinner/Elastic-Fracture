...
for j = 1:n-1
    ...
    Kmatrix(2*n-1,j) = uK11aint - lK11aint;
	Kmatrix(2*n-1,(n-1)+j) = uK11bint - lK11bint;
	Kmatrix(2*n-1,2*(n-1)+j) = uK12cint - lK12cint;
	Kmatrix(2*n-1,3*(n-1)+j) = uK12dint - lK12dint;
end
...
Kmatrix(2*n-1,4*n-3) = uK11bint - lK11bint;
Kmatrix(2*n-1,4*n-2) = uK12dint - lK12dint;

%accounting for the crack
lx = 0;
ux = inpoint(1);
[lK11aint, lK11bint, lK12cint, lK12dint, lK21aint, lK21bint, lK22cint, lK22dint] = linkernelmatrices(lx,x);
[uK11aint, uK11bint, uK12cint, uK12dint, uK21aint, uK21bint, uK22cint, uK22dint] = linkernelmatrices(ux,x);
Kmatrix(2*n-1,4*n-1) = uK11bint - lK11bint;
Kmatrix(2*n-1,4*n) = uK12dint - lK12dint;

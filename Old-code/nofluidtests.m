...
%vector of h' points
%inpoint = endpoint*((1:n)/n).^2;
inpoint = tan((1:n)*(atan(endpoint))/n);
%inpoint = tan((0.5:1:n-0.5)*(pi/(2*(n+1))));

%vector of h points
%outpoint = endpoint*((0.5:1:n-0.5)/n).^2;
outpoint = tan((0.5:1:n-1.5)*(atan(endpoint))/n);
...
gcoeffmatrix = vandermonde(inpoint(n-3:n));
gcoeffmatrix = gcoeffmatrix*[inpoint(n-3)^2,0,0,0; ...
    0,inpoint(n-2)^2,0,0;0,0,inpoint(n-1)^2,0;0,0,0,inpoint(n)^2;];
interpol(4*nfin+2:4*nfin+5,n-3:n) = gcoeffmatrix;

%use ax^2 + bx + c + d/x + e/x^2 for h'
hcoeffmatrix = vandermonde(inpoint(n-4:n));
hcoeffmatrix = hcoeffmatrix*[inpoint(n-4)^2,0,0,0,0; ...
    0,inpoint(n-3)^2,0,0,0;0,0,inpoint(n-2)^2,0,0;0,0,0,inpoint(n-1)^2,0;...
    0,0,0,0,inpoint(n)^2];
interpol(4*nfin+6:4*nfin+10,2*n-4:2*n) = hcoeffmatrix;
    


%creates a matrix to find pressure
Kmatrix = zeros(2*(n-2), 4*nfin+10);
for i = 1:n-2
    x = outpoint(i);
    ...
    Kmatrix(n-2+i, 4*nfin+5) = uK21iint - lK21iint;
    
    Kmatrix(n-2+i, 4*nfin+6) = uK22fint - lK22fint;
    Kmatrix(n-2+i, 4*nfin+7) = uK22cint - lK22cint;
    Kmatrix(n-2+i, 4*nfin+8) = uK22dint - lK22dint;
    Kmatrix(n-2+i, 4*nfin+9) = uK22hint - lK22hint;
    Kmatrix(n-2+i, 4*nfin+10) = uK22jint - lK22jint;
...
%the constant coeff
ginfty(1,n-3:n) = gcoeffmatrix(2,:);
%the linear coeff
ginfty(2,n-3:n) = gcoeffmatrix(1,:);

%conditions on h' = ax^2 + bx + c + d/x, i.e. on an a,b
hinfty = zeros(2,2*n);

%the linear coeff
hinfty(1,2*n-4:2*n) = hcoeffmatrix(2,:);
%the quadratic coeff
hinfty(2,2*n-4:2*n) = hcoeffmatrix(1,:);
...
%plots implied pressure distribution

%number of points in the plot
measurepoint = tan((0.05:0.1:n-0.05)*(atan(endpoint))/n);
m = length(measurepoint);
...
for i = 1:length(measurepoint)
    ...
    pressure(i) = pressurerow*interpol*soln;
    shear(i) = shearrow*interpol*soln;
end

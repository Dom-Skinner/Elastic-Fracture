...
R_t = lubrication_integral(x,z,n,t,h_coefficient_matrix_s,h0,h0_z,l0,s);
...
P_t = R_t*Hprime_tilde_s;
Pprime_t = (P_t(2:end)-P_t(1:end-1))'./(z(2:end)-z(1:end-1));
...
plot(z(1:n-2),z(1:n-2).^(2-s).*Pprime_s,'o',z(1:n-2),z(1:n-2).^(2-s).*Pprime,'o',...
    z(1:n-2),z(1:n-2).^(2-s).*Pprime_t,'*',x,x.^(2-s).*Pprime_tilde)

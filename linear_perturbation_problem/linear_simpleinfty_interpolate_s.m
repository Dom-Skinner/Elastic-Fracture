...
% Careful - h' and g' interpolated differently

function interpolate_matrix = linear_simpleinfty_interpolate_s(x,s)
...
interpolate_matrix(2*n+t-1,n+t) = x(t)^(1-s)/(x(t)-x(t-1));
...
interpolate_matrix(3*n+t-1,n+t) = -x(t-1)*x(t)^(1-s)/(x(t)-x(t-1));

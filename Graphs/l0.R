...
p <-ggplot(data=dat, aes(x=K, y= lambda,group=type,shape=type,color=type,linetype=type))  +
  geom_line( size=0.4) +
  geom_point(size=0.9) +
  ...
  coord_cartesian(ylim=c(0.0569,0.0593),xlim=c(0,0.275),expand=FALSE) +
  ylab(expression(lambda)) + xlab(expression(K[I]^u)) +
  scale_y_continuous(breaks= seq(0.057,0.059,by=0.0005), 
                     labels = insert_minor( seq(0.057, 0.059, by=0.0005), 0 ))+ 

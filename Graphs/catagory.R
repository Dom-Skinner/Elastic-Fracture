...
p<-ggplot(data=dat, aes(x=KI, y= KII))  +
      ...
      geom_hline(aes(yintercept=dat$KII[length(dat$KII)]), size=0.3)+
      annotate("segment", x=1.95, xend=3, y=1.505, yend=1.505, size=0.3)+
      geom_linerange(aes(x=dat$KI[1], y=NULL, ymin=dat$KII[1], ymax=4), size=0.3)+
      geom_segment(data=arrow_data, mapping=aes(x=x, y=y, xend=x+vx, yend=y+vy), 
      arrow=arrow(length=unit(1.2, "mm"),type = "closed"), size=0.3) +
